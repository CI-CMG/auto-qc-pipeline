...
class RouteConfigurer(object):
  ...
  def __init__(self, eip_context, wod_directory, auto_qc_home, gunzip_directory,
    ...
    self.__gunzip_processor = FileGunzipProcessor(wod_directory,
                                                  gunzip_directory)
    self.__wodpy_processor = WodpyProfileProcessor(self.__test_queue)
    self.__test_processor = ProfileTestProcessor(auto_qc_home,
  ...
  def configure(self):
    ...
    self.__eip_context.add_route_builder(
      ProfileTestRoute(self.__test_concurrency, self.__error_queue,
